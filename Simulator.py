...
class MatrixSimulator:
    def __init__(self, root):
        ...
        # Ensure grid is drawn after the window is fully laid out (macOS fix)
    ...
    def create_widgets(self):
        ...
        # Left: Canvas
        self.canvas = tk.Canvas(main_pane, bg="#0d0d0d", highlightthickness=0)
    ...
    def draw_grid(self):
        ...
        for y in range(BOARD_HEIGHT):
            for x in range(BOARD_WIDTH):
                ...
                display_col = hex_col if (color != (0,0,0)) else "#1a1a1a"
                self.rects[(x, y)] = self.canvas.create_rectangle(
                    x1, y1, x2, y2, fill=display_col, outline="white" if is_triggered else "#333333", width=2 if is_triggered else 1
                )
    ...
    def update_pixel(self, x, y, r, g, b, timestamp=None):
        if (x, y) in self.rects:
            ...
            self.canvas.itemconfig(self.rects[(x, y)], fill=display_col, outline="white" if is_triggered else "#333333", width=2 if is_triggered else 1)
