...
class LastLight:
    ...
    def __init__(self):
        ...
        if pygame:
            try:
                pygame.mixer.init()
                bgm_path = os.path.join(os.path.dirname(__file__), "sounds", "horror_bgm.mp3")
    ...
    def _reset(self):
        ...
        if getattr(self, 'bg_music_loaded', False):
    ...
    def _game_loop(self):
        ...
        if getattr(self, 'bg_music_loaded', False):
            pygame.mixer.music.play(-1)  # loop endlessly
    ...
    def _ending(self, instant_fail=False):
        ...
        if instant_fail:
            ...
            # Dramatic jumpscare / heavy hit if pygame works
            if getattr(self, 'bg_music_loaded', False):
                pygame.mixer.music.stop()
            
            if winsound:
                threading.Thread(target=lambda: [winsound.Beep(100, 300), winsound.Beep(50, 800)], daemon=True).start()
